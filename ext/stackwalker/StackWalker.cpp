...
char* StackWalker::ShowCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  ...
  for (frameNum = 0; ; ++frameNum )
  {
    ...
    if (s.AddrPC.Offset != 0)
    {
      ...
      if (this->m_sw->pSGSFA(this->m_hProcess, s.AddrPC.Offset, &(csEntry.offsetFromSmybol), pSym) != FALSE)
      {
		...
		//strcpy(&outbuffer[strlen(outbuffer)],pSym->Name);
      ...
      { // yes, we have SymGetLineFromAddr64()
        if (this->m_sw->pSGLFA(this->m_hProcess, s.AddrPC.Offset, &(csEntry.offsetFromLine), &Line) != FALSE)
        {
		  ...
		  if(frameNum>1)
		  {
		  strcpy(&outbuffer[strlen(outbuffer)],csEntry.name);
		  strcpy(&outbuffer[strlen(outbuffer)]," [");
	  	  strcpy(&outbuffer[strlen(outbuffer)],csEntry.lineFileName);
		  strcpy(&outbuffer[strlen(outbuffer)],"]\n");
		  }
        }
        else
