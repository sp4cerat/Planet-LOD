		...
		vec3d center, double size = 1)
	{
		...
		{ 
			if (gui.screen[0].checkbox["patches"].checked)
				draw_patch(p1, p2, p3,uv1,uv2,uv3); 
			else
				draw_triangle(p1, p2, p3, uv1, uv2, uv3);
			return; 
		}
		// Recurse
...
void render_callback(Gui::Window *window, Gui::Button* control, int index)
{
	...
	static uint t0 = timeGetTime(), t1 = 0; float dt;
	t1 = t0; t0 = timeGetTime(); dt = t0 - t1;
	...
	float speed = (0.001 + max(0, pos.len() - 1.0)) * 0.02*(dt / 100);

	if (b.hover)										// zoom by wheel
	{
		pos = pos + vec4f(dir_z.x, dir_z.y, dir_z.z, 0)*(float(gui.mouse.wheel)*speed*50.0);
		gui.mouse.wheel = 0;
	}
	...
	gluPerspective(z, (GLfloat)b.sx / (GLfloat)b.sy, 0.00001, 4.0);
...
void init_gui()
{
	...
	int y = 20;
	w.label["detail"] = Gui::Label("Detail : 0.00001",20, y, 100, 20);
	w.slider["detail"] = Gui::Slider(0.00001, 1, 0.00001, 120, y, 100, 20);
